...
Few assumptions:
    ...
    - Else, checks the list for an increasing list ACCOUNTING FOR DUPLICATES (no specifics given in the problem statement besides increasing sorted order...)
-}
...
select :: (a -> Bool) -> [a] -> [b] -> [b]
select pred is xs = 
    selectHelper 0 pred is xs

-- >>> select odd [1..100] "hello world this is joe"
-- "hlowrdti sje"

selectHelper _ _ [] [] = []
selectHelper _ _ [] _ = []
selectHelper _ _ _ [] = []
selectHelper count pred (is:iss) (x:xs)
    | (pred is) =
        x : selectHelper (count + 1) pred iss xs
    | otherwise = 
        selectHelper (count + 1) pred iss xs
...
-- select pred is xs = 
--     selectHelper minLen 0 pred is xs
--     where minLen = min (length is) (length xs)

-- selectHelper :: (Ord a1, Num a1) => a1 -> a1 -> (t -> Bool) -> [t] -> [a2] -> [a2]
-- selectHelper _ _ _ [] [] = []
-- selectHelper _ _ _ [] _ = []
-- selectHelper _ _ _ _ [] = []
-- selectHelper minLen count pred (is:iss) (x:xs)
--     | (count < minLen) =
--         if (pred is)
--             then
--                 x : selectHelper minLen (count + 1) pred iss xs
--                     else
--                         selectHelper minLen (count + 1) pred iss xs
--     | otherwise = []
{-
Note: Otherwise case is not needed since it is covered in the first three pattern matchings
-}
