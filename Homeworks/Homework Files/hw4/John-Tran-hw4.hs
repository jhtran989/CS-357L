...
A list with just element just returns a leaf (see sample test cases above).
-}
...
{-
Note: see the end for an alternate version that matches the given sample output

IMPORTANT: the sample output given for example2 (below) in the homework specifications makes it seem like the path is constructed from the LEAF to the ROOT, which we think there might be a typo since it should be paths STARTING AT THE ROOT. If we look specifically at the "GoRight (GoLeft This)" case, then we see that the tree is traversed to the left first. But by that point, a leaf node is reached and no further actions could be taken.
...
{-
Assume elements of the list cannot be used more than once (only way for duplicates to appear in the combos is if there were duplicates in the original list -- e.g., "AABCD")

General approach is to shrink each nested loop by removing elements currently being used in the outer loops -- INEFFICIENT. However, wanted to see how a list comprehension could be used to solve this. The annoying part is the lack of modularity as we have to know HOW MANY ELEMENTS we want in the combo and with the lack of index tracking in Haskell, we had to get a workaround with "zip" and the built-in enumeration. Since we don't want to overcount combos, we only need to check elements AFTER the previous outer loops in the original list -- that is why "drop" was used. 
...
{-
Referenced the in-class lecture "powerset" code where the length of the subset differs when the current element is being used (decrease the subset length by 1) or when it was not used (subset length remains the same as the current call)

Note: if the given list length is greater than the given list, then it just returns the empty list (no subsets can be created); as a base case for a call with a list length of 0, the two nested empty list is used so the structure of the list is maintained (i.e., map can be used in the INNER empty list while the outer list is kept the same)
-}
...
{-
Again, rather trivial. Just followed the format of Eq and the definition provided
-}

instance Show Complex where
    show (Complex real imaginary) = (show real) ++ "+" ++ (show imaginary) ++ "i"
{-
Again, rather trivial. Just followed the format of Show and the definition provided
-}
...
{-
Again, rather trivial. Just followed the format of Num and the definition provided
-}
